...
def test_transform_logging():
    ...
    if costs["hp_cost"] > 0:
        cost_parts.append(f"❤️ {costs['hp_cost']}")
    if costs["star_cost"] > 0:
        cost_parts.append(f"⭐ {costs['star_cost']}")
    ...
    print(f"  {log_message}")
    print("\n✅ PASS: Log includes costs and stat changes in one line")
...
def test_attack_auto_damage():
    ...
    for attack_type in ["light", "medium", "heavy"]:
        ...
        if auto_damage == expected_damage:
            ...
            print(f"  ❌ Calculation wrong (expected {expected_damage})")
            all_passed = False
...
def test_damage_threshold():
    ...
    for damage, description in scenarios:
        ...
        if threshold_exceeded:
            print(f"  ⚠️  Exceeds threshold ({threshold_damage})")
            print(f"  Warning: {target} must make DC {threshold_dc} CON save")
            print(f"  ✅ Would show threshold warning in embed")
        else:
            print(f"  ✓ Below threshold ({threshold_damage})")
...
def test_mp_scaling():
    ...
    for mp_spent, expected_hp_bonus, expected_stars_bonus in scenarios:
        ...
        if hp_bonus == expected_hp_bonus and stars_bonus == expected_stars_bonus:
            ...
            print(f"  ❌ Scaling wrong (expected +{expected_hp_bonus} HP, +{expected_stars_bonus} Stars)")
